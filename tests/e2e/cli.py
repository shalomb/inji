...
def run_negative_test(
    command=[injicmd], exit_code=2, errors=None, stderr=subprocess.STDOUT, input=None
):
    ...
    with pytest.raises(NegativeTestException) as e_info:
        try:
            ...
            msg = f"exit_code:{exc.returncode} output:{exc.output}"
...
class TestInjiCmd(unittest.TestCase):
    ...
    def test_stdin(self):
        ...
        assert (
            check_output(injicmd, input=b"{% set foo='world!' %}Hola {{ foo }}") == "Hola world!\n"
        )
    ...
    def test_json_config_args(self):
        ...
        assert (
            check_output(injicmd, "-j", '{"foo": "world!"}', input=b"Hola {{ foo }}")
            == "Hola world!\n"
        )
    ...
    def test_invalid_json_config_args(self):
        ...
        for json_input in input_cases:
            run_negative_test(
                command=[injicmd, "-j", json_input],
    ...
    def test_kv_config_args(self):
        ...
        assert (
            check_output(
                ...
                "bar=bar",
                "-k",
                "foo=bar",  # should keep bar
                "-k",
                "foo=world!",  # valid, last declaration wins
                "-k",
                "moo=",  # valid, sets an empty moo
    ...
    def test_invalid_kv_config_args(self):
        ...
        for kv in input_cases:
            run_negative_test(command=[injicmd, "-k", kv], errors=["Invalid key found parsing"])
    ...
    def test_12factor_config_sourcing(self):
        ...
        file_from_text("zar: bella\nzoo: cfg", dir=tmpdir, name="inji.yml")

        param_file = file_from_text("zar: zella\nzoo: zorg", dir=tmpdir, name="vars.yaml")
        ...
        assert re.search(
            r"Hola \w+ from bella",
            check_output(injicmd, "-o", f"{tmpdir}/dev", input=b"Hola {{ item }} from {{ zar }}"),
        )

        # dev/c.yaml should be last file sourced
        assert (
            check_output(injicmd, "-o", f"{tmpdir}/dev", input=b"Hola {{ zoo }}") == "Hola c.yaml\n"
        )
    ...
    def test_template_missing(self):
        ...
        run_negative_test(
            ...
            errors=["nonexistent-template.j2.. does not exist"],
        )
    ...
    def test_template_render_with_varsfile(self):
        ...
        assert check_output(injicmd, template, "-v", varsfile) == "Hola world!\n"
    ...
    def test_template_render_with_multiple_varsfiles(self):
        ...
        assert (
            check_output(injicmd, template, "-v", varsfile1, "-v", varsfile2)
    ...
    """
        assert run_negative_test(
            command=[injicmd, file_from_text("Hola {{ foo }}"), "-v", file_from_text("")],
            exit_code=1,
            errors=["TypeError: .* contains no data"],
        )
    ...
    def test_filters_get_and_format_dict(self):
        ...
        assert (
            check_output(
                injicmd,
                "-k",
                "url=https://google.com:443/webhp?q=foo+bar",
                file_from_text("""{{
            url | urlsplit |
    ...
    """)
        ...
        output = check_output(
            injicmd,
            file_from_text(f"""{{{{ markdown('{md_file}') }}}}"""),
        )
        print(output)
    ...
    """)
        ...
        output = check_output(
            ...
            file_from_text(
                f"""
          {{{{ markdown('{md_file}', extensions=['codehilite', 'extra']) }}}}
          """
            ),
        )
    ...
    def test_globals_strftime(self):
        ...
        assert re.search(
            ...
            check_output(
                injicmd,
                file_from_text("""{{ date | strftime("%FT%T") }}"""),
            ),
        )
